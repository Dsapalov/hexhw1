...
public class MainActivity extends AppCompatActivity {
    ...
    void startCalculation() {
        ...
        for (int i = 0; i < vectors.size(); i++) {
            // 1 +
//            hexToLittleEndian(vectors.get(i));
            // 2
            hexToBigEndian(vectors.get(i));

            // 3 +
//            littleEndianToHex(vectors.get(i));
            // 4 +
//            bigEndianToHex(vectors.get(i));
        }
...
//        vectors.add(fourthVector);
    ...
    void hexToLittleEndian(Vector inputVector) {
        long ret = 0;
        ...
        System.out.println("---> hexToLittleEndian result: " + ret);
    }

    void hexToBigEndian(Vector inputVector) {
        long ret = 0;
        String hexLittleEndian = "";
        if (inputVector.hexValue.length() % 2 == 0) {
            try {
                ret = Long.parseLong("ff", 16);
            } catch (Exception ex ){
                System.out.println("---> parseLong ex: " + ex.toString());
            }
        }

        System.out.println("---> hexToBigEndian result: " + ret);
    }

    void littleEndianToHex(Vector inputVector) {

        BigInteger big = new BigInteger(inputVector.littleEndian);
        String result = big.toString(16);
        System.out.println("---> littleEndianToHex result: " + result);
    }

    void bigEndianToHex(Vector inputVector ) {
        BigInteger big = new BigInteger(inputVector.bigEndian);
        String result = big.toString(16);
        System.out.println("---> bigEndianToHex result: " + result);
    }
